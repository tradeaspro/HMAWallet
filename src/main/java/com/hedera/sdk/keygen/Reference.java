...
public class Reference {
    ...
    /**
     * use this for all logging, as controlled by the optional data/log4j2.xml
     * file
     */

    /**
     * Pass to the constructor 16, 32, or 48 bytes (128, 256, or 384 bits),
     * which is the hash of the thing being referenced (a key, a swirld name, a
     * file, etc). A copy of data is made, so it is OK to change the array after
     * instantiating the Reference.
     *
     * The stored information will be one byte longer than the data, found by
     * first appending a CRC8 checksum to the end, then XORing all the previous
     * bytes with that checksum. Later, the checksum can be checked by doing the
     * XOR, then the checksum calculation, then comparing the result to the last
     * byte. When expressing a Reference in base 62 or as a list of words, the
     * longer version is used.
     *
    ...
    /**
     * Pass to the constructor a string representing the data. If the string
     * starts and ends in <> and contains only letters and digits in between,
     * then it is considered a base62 encoding. Otherwise, it is considered a
     * list of words.
     *
    ...
    /**
     * Given a number in base fromRadix, with each digit being an integer in
     * array number (number[0] is most significant), return a new number in
     * toRadix, where the array has toLength elements, left padded with zeros if
     * the number is too small, and chopping off the most significant digits if
     * it is too large.
     *
     ...
     * Koopman says 0xA6 (1 0100110) is a good polynomial choice, which is x^8 +
     * x^6 + x^3 + x^2 + x^1 . The following code uses 0xB2 (1 0110010), which
     * is 0xA6 with the bits reversed (after the first bit), which is needed for
     * this code.
     *
    ...
    /**
     * Return a string representing the reference, made up of multiple lines of
     * 4 words each, broken into groups of 4 lines. The last group may have
     * fewer lines, and its last line may have fewer words.
     *
    ...
    /**
     * Return a string representing the reference, made up of multiple lines of
     * 4 words each, broken into groups of 3 lines. The last group may have
     * fewer lines, and its last line may have fewer words. There are no actual
     * line breaks unless they are included in the parameters passed in.
     *
     * @param prefix start of the entire string
     * @param wordSeparator between adjacent words in a line of 4 words
     * @param lineSeparator1 between lines in a group of 3 lines, for every
     * other group, starting with first
     * @param lineSeparator2 between lines in a group of 3 lines, for every
     * other group, starting with second
    ...
    /**
     * return a string representation of the given byte array in hex, with a
     * each byte becoming two characters (including a leading zero, if
     * necessary). Only use the array from index firstIndex to index lastIndex.
     * A lastIndex of -1 means the last element, -2 is the second to last, and
     * so on. If minIndex>maxIndex (after any wrapping of lastIndex), then
     * return the null string.
     *
     * @param bytes an array of bytes, some of which are to be converted
     * @param firstIndex index of first element to convert
     * @param lastIndex index of last element to convert (or -1 for last -2 for
     * 2nd to last, etc)
